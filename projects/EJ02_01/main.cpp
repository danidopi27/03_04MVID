...
void render(uint32_t VAO, uint32_t program) {
    ...
    glDrawArrays(GL_TRIANGLES, 0, 6);								// Se dibujan los elementos
}
//----------------------------------------FUNCIÓN RENDER----------------------------------------//
...
int main(int, char*[]) {
    ...
    uint32_t VBO;											// Creación del VBO
    const uint32_t VAO = createVertexData(&VBO);			// Creación de los vértices
