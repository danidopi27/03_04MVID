...
glm::vec3 lightDir(3.0f, -1.0f, 0.0f);
const float k_lightSpeed = 0.5f;
float angulo = 0.0f;
...
void render(const Geometry& floor, const Geometry& object, const Geometry& light, const Shader& s_phong, const Shader& s_light,
	...
	angulo += 0.1;
	lightDir = dt * k_lightSpeed * glm::vec3(glm::cos(angulo), 0.0f, glm::sin(angulo));
...
int main(int, char* []) {
	...
	while (window->alive()) {
		...
		render(quad, teapot, sphere, s_phong, s_light, t_albedo, t_specular, deltaTime);
		window->frame();
	}
