...
#include "engine/camera_06_01.hpp"			// Librería de manejo de cámara modificada
...
//--------------------------------------------VARIABLES--------------------------------------------//
Camera_06_01 camera(glm::vec3(0.0f, 0.0f, 3.0f));		// Definición de la cámara
...
void handleInput(float dt) {
	...
	if (input->isKeyPressed(GLFW_KEY_W)) {
		camera.handleKeyboard(Camera_06_01::Movement::Forward, dt);		// Movimiento hacia adelante
	}
	// Si la tecla S está pulsada
	if (input->isKeyPressed(GLFW_KEY_S)) {
		camera.handleKeyboard(Camera_06_01::Movement::Backward, dt);	// Movimiento hacia atrás
	}
	// Si la tecla A está pulsada
	if (input->isKeyPressed(GLFW_KEY_A)) {
		camera.handleKeyboard(Camera_06_01::Movement::Left, dt);		// Movimiento hacia la izquierda
	}
	// Si la tecla D está pulsada
	if (input->isKeyPressed(GLFW_KEY_D)) {
		camera.handleKeyboard(Camera_06_01::Movement::Right, dt);		// Movimiento hacia la derecha
	}
}
//------------------------------------------FUNCIÓN HANDLEINPUT------------------------------------------//
