...
uint32_t createVertexData(uint32_t* VBO, uint32_t* EBO, float radio) {
	// Creación de los vertices de los triangulos
	float vertices[(LADOS + 1)*3];														// Array para los 7 vértices (18 coordenadas)							

	vertices[0] = 0.0f;																	// Coordenada en x del vértice central
	vertices[1] = 0.0f;																	// Coordenada en y del vértice central
	vertices[2] = 0.0f;																	// Coordenada en z del vértice central

	float delta_angulo = ((360.0 / LADOS) * PI)/180;									// Obtención de incremento de ang. en rad
	float angulo_actual = 0;															// Ángulo inicial es 0 rad

	float x = 0;																		// Inicialización de coordenada en x
	float y = 0;																		// Inicialización de coordenada en y
	int vertice_actual = 0;																// Vértice actual es el central

	// Se recorren los vertices externos del hexágono
	for (int i = 1; i<=LADOS; i++)
	{
		x = cos(angulo_actual) * radio;													// Conversión de coordenada polares al eje x
		y = sin(angulo_actual) * radio;													// Conversión de coordenada polares al eje y

		vertice_actual = i * 3;															// Posición de coordenadas del vértice actual

		vertices[vertice_actual] = x;													// Coordenada en x
		vertices[vertice_actual + 1] = y;												// Coordenada en y 
		vertices[vertice_actual + 2] = 0.0f;											// Coordenada en z es 0

		angulo_actual += delta_angulo;													// Se pasa al siguiente ángulo
	}
...
int main(int, char* []) {
	...
	float radio = 0.5;
	const uint32_t VAO = createVertexData(&VBO, &EBO, radio);		// Creación de los vértices
