...
void render(const Geometry& floor, const Geometry& object, const Shader& s_phong, const Shader& s_blend, 
	...
	for (const auto& cubePos : cubePositions)
	{
		for (int i = 0; i < 3; i++) {
			s_blend.use();
			model = glm::mat4(1.0f);
			model = glm::translate(model, glm::vec3(cubePos.x - 0.75f + 0.75f * i, cubePos.y, cubePos.z + 0.75f + 0.1 * i));
...
int main(int, char* []) {
    ...
    const Shader s_phong("../projects/EJ11_01/phong.vs", "../projects/EJ11_01/blinn.fs");		// Carga del shader de phong
    const Shader s_blend("../projects/EJ11_01/blend.vs", "../projects/EJ11_01/blend.fs");		// Carga del shader de blend
    ...
    const Texture t_tree("../assets/textures/grass.png", Texture::Format::RGBA);					// Textura de arbol
    const Cube cube(1.0f);																		// Creaci�n de cubo
	...
	while (window->alive()) {
        ...
        render(quad, cube, s_phong, s_blend, t_albedo, t_specular, t_tree);						// Se renderiza
