...
void render(const Geometry& quad, const Shader& s_phong, const Shader& s_blend, const Texture& t_albedo, const Texture& t_specular) {
	...
	for (int i = 0; i < 6; i++)
	{
		...
		else
		{
			...
			s_blend.set("material.ambient", 0.1f * quadColor[i - 3]);
			s_blend.set("material.diffuse", 0.5f * quadColor[i - 3]);
			s_blend.set("material.specular", 1.0f * quadColor[i - 3]);
			s_blend.set("material.shininess", 32);

			s_blend.set("material.alpha", quadAlpha[i - 3]);
