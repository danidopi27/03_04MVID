...
//-------------------------------------------------------------------FUNCI�N RENDER-------------------------------------------------------------------//
void render(const Geometry& quad, const Geometry& cube, const Shader& s_phong, const Shader& s_fbo, const Shader& s_blur_horizontal, const Shader& s_blur_vertical,
	const Texture& t_albedo, const Texture& t_specular, const uint32_t fbo, const uint32_t fbo_blur, const uint32_t fbo_texture, const uint32_t fbo_blur_texture) {
	...
	s_blur_horizontal.use();

	glActiveTexture(GL_TEXTURE0);
	glBindTexture(GL_TEXTURE_2D, fbo_blur_texture);
	s_blur_horizontal.set("screenTexture", 0);
	...
	glActiveTexture(GL_TEXTURE0);
	glBindTexture(GL_TEXTURE_2D, fbo_blur_texture);
	s_blur_vertical.set("screenTexture", 0);
...
int main(int, char* []) {
	...
	const Shader s_phong("../projects/EJ13_03/phong.vs", "../projects/EJ13_03/blinn.fs");		// Carga del shader de phong
	const Shader s_fbo("../projects/EJ13_03/fbo.vs", "../projects/EJ13_03/fbo.fs");				// Carga del shader de fbo
	const Shader s_blur_horizontal("../projects/EJ13_03/blur.vs", "../projects/EJ13_03/blur_horizontal.fs");
	const Shader s_blur_vertical("../projects/EJ13_03/blur.vs", "../projects/EJ13_03/blur_vertical.fs");
	...
	while (window->alive()) {
		...
		render(quad, cube, s_phong, s_fbo, s_blur_horizontal, s_blur_vertical, t_albedo, t_specular, 
			std::get<0>(fbo1), std::get<0>(fbo1), std::get<1>(fbo1), std::get<1>(fbo2));				// Se renderiza
